...
class ObjectDetector(BaseObjectDetector):
    ...
    def __init__(self):
        ...
        self.classes = 1  # filter by class: --class 0, or --class 0 2 3
...
#------ Realsense camera configuration, pipeline, and 
    ...
    config = rs.config()
    config.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30) # set realsense color configuration
    config.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30) # set realsense depth configuration
    ...
    while True:
        ...
        for track in active_tracks:
            if track[0].class_id == 2:
                # Pass class 2 detections to QuaternionEstimator
                crop_img_before = img[int(track[1][1]):int(track[1][3]), int(track[1][0]):int(track[1][2])]
                cropped_image = estimator.crop(crop_img_before, int(track[1][0]), int(track[1][1]), int(track[1][2]), int(track[1][3]))
                quaternions = estimator.predict(cropped_image)
        
                # Print estimations for class 2 detections
                print("Estimations for Class 2 Detections:")
                for quat in quaternions:
                    print(quat)
            
                # Process quaternions as needed for class 2 detections

            elif track[0].class_id == 1:
            ...
            for track2 in active_tracks2:
                ...
                # display the coordinates on the original 640 by 480 image
